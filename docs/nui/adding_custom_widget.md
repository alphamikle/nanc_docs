...
That's it! If we were creating a fairly simple widget that has no descendants, and no complex parameters, then that would be the end of our custom renderer creation, and we would only have to use it in the [Nanc configuration](../nanc_configuring), and the [`NUIWidget`](../mobile_app_configuring) mobile application widget.
